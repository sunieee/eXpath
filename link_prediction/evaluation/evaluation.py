...
class Evaluator:
    ...
    def evaluate(self,
                samples: np.array,
                write_output:bool = False, folder='.', type='all'):
        
        print('evaluate samples:', samples.shape)
        if not self.args.specify_relation:
            return self._evaluate(samples, write_output, folder, print_prefix='\t', type=type)
        ...
        for i in range(len(index[:-1])):
            ...
            ret[rel] = self._evaluate(batch, write_output, 
                ...
                print_prefix=self.dataset.relation_id_2_name[rel], type=type)
    ...
    def _evaluate(self,
                samples: np.array,
                write_output:bool = False, folder='.', print_prefix='\t', type='all'):
        self.model.cuda()
